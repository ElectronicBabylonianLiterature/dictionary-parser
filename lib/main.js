...
module.exports = async function main (fileName) {
  try {
    ...
    const broken = {
      ...
      lostaAplifiedMeaning: dictionary.entries.filter(checks.hasLostAmplifiedMeaning),

      get size () {
        return _.sum([this.forms.length, this.derived.length, this.amplifiedMeaning.length, this.lostaAplifiedMeaning.length])
      }
    }
    ...
    console.log(
      ...
      '\n✔️ ', dictionary.entries.length - broken.size,
      ' 🚧 ', broken.size,
