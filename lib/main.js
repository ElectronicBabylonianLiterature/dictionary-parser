...
function logResults (dictionary, broken) {
  console.log(
...
async function save (dictionary, broken) {
  await Promise.all([
...
module.exports = async function main (dictionaryFileName, rootsFileName) {
  try {
    ...
    const broken = getBroken(dictionary)
    logResults(dictionary, broken)
    await save(dictionary, broken)
  } catch (error) {
    console.log(error)
  }
}
