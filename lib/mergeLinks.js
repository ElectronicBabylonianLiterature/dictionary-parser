...
module.exports = function mergeLinks (entries, links) {
  ...
  const bar = createProgressBar('Merging links', entries.length)
  ...
  const unlinked = _(derivedFormsGroups)
    .flatten()
    .reject(derivedForm => derivedForm.linked)
    .map(derivedForm => derivedForm.value.source)
    .uniq()
    .value()
