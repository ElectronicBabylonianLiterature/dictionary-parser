...
function parseNestedWordArray (string, trim) {
  return parseSubArray(string, ';', trim).filter(_.negate(_.isEmpty)).map(element => parseWordArray(element, trim))
}
...
module.exports = class Entry {
  ...
  get derived () {
    ...
    return rawDerived
      ? parseNestedWordArray(rawDerived, ' *\\>')
      : defaultDerived
  }
}
