...
const dictionaryRowRegExp = /^\*\*(?<notAttested>\\\*)?(?<lemma>.+?)\*\*;?(?:\s(?<homonym>[IV]+);?)?(?<forms>(?:[,;]\s[^*"]*(?:\\\*)?\*[^*"]+\*[^*"]*?;?)+)?\s?(?<meaning>(?:"|'|\(|\w|\?[^);]|\\~|\\\[|\*cf.\*).+?)?(?<amplifiedMeanings>\s\*\*(?:\w+\.|G|Gtn|Gt|D|Dtn|Dt|Dtt|Š|Štn|Št|ŠD|N|Ntn|R|Št2)\*\*.+?;?)?\s*(?<derived>(?:\\>|\\<).+?)?(?<cf>; \*cf\.\* [^<>]+)?$/
...
module.exports = class Entry {
  ...
  get forms () {
    ...
    const alternatives = _.tail(this.lemmaAlternatives).map(lemma => ({lemma: lemma, notes: [], attested: this.attested}))
  ...
  get derived () {
    ...
    const derived = derivedMatch ? parseDerived(derivedMatch[1]) : defaultDerived

    if (this.match.groups.cf) {
      return derived.concat(parseDerived(this.match.groups.cf))
    } else {
      return derived
    }
  }
  ...
  get derivedFrom () {
    const rawMatch = this.match.groups.derived || ''
    const derivedFromRegExp = /(?:\\>[^<]*)?\\<\s*\*(?<lemma>[^\\+]+)\* ?(?<homonym>[IV]+)? ?(?<postNote>[^;]+)?;?\s*(?:\\>)?/
    ...
    return derivedFromMatch
      ? {
        ...
        notes: derivedFromMatch.groups.postNote ? [derivedFromMatch.groups.postNote.trim()] : []
      }
      : defaultDerivedFrom
  }
