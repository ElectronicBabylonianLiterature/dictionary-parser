...
const dictionaryRowRegExp = /^\*\*(?<notAttested>\\\*)?(?<lemma>.+?)\*\*;?(?:\s(?<homonym>[IV]+))?(?<forms>(?:[,;]\s[^*"]*(?:\\\*)?\*[^*"]+\*[^*"]*?)+)?\s?(?<meaning>(?:"|'|\(|\w|\?|\\~|\\\[|\*cf.\*).+?)?(?<amplifiedMeanings>\s\*\*(?:\w+\.|G|Gtn|Gt|D|Dtn|Dt|Dtt|Š|Štn|Št|ŠD|N|Ntn|R)\*\*.+?;?)?\s*(?<derived>(?:\\>|\\<).+?)?$/
