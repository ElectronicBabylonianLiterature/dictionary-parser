...
const dictionaryRowRegExp = /^\*\*([^\s]+?)\*\*(\s([IV]+))?((?:[,;]\s[^*"]*(?:\\\*)?\*[^*"]+\*[^*"]*?)+)?\s?((?:"|'|\(|mng\.|f\.|m\.|pl\.|sg\.|in|Bab\.|jB|O\/jB|OB).+?)?(\s\*\*(?:\w+\.|\\~|G|Gtn|Gt|D|Dtn|Dt|Dtt|Š|Štn|Št|ŠD|N|Ntn|R)\*\*.+?;?)?\s*((?:\\>|\\<).+?)?$/
const lemmaIndex = 1
...
module.exports = class Entry {
  ...
  get isLink () {
    const linkRexExp = /^(?:\*\*(?:\\\*)?[^\s]+\*\*(?:, )?)+\s+(?:etc\.\s+)?\*cf\.(?:\*\s+(?:also\s+)?\*|\s+)[^\s]+\*(?:\s+\w+)?/
